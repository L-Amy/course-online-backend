...
module.exports = {
    ...
    //提问问题
    async askTask(request) {
        let createDate = fecha.format(new Date(), 'YYYY-MM-DD HH:mm:ss')
        let sql = `INSERT INTO discuss (TaskId,StudentId,DisscusQuestion,TeacherId,CreateDateTime)VALUES(${request.Id},${request.StudentId},'${request.AnswerContent}',${request.TeacherId},'${createDate}')`;
        return mysql.createConnection(DBConfig).then(conn => {
            var result = conn.query(sql);
            conn.end();
            return result;
    ...
    async getAskList(request) {        
        if (request.TeacherId > 0 && request.StudentId > 0) {
            var sql = `SELECT * FROM discuss WHERE StudentId=${request.StudentId} and TeacherId=${request.TeacherId}`;
        }else if (request.StudentId > 0) {
            var sql = `SELECT * FROM discuss WHERE StudentId=${request.StudentId}`;
        }
        return mysql.createConnection(DBConfig).then(conn => {
            var result = conn.query(sql);
            conn.end();
    ...
    async getAskDetail(id) {
        let sql = `SELECT * FROM discuss WHERE Id=${id}`;
        return mysql.createConnection(DBConfig).then(conn => {
            var result = conn.query(sql);
            conn.end();
            return result;
    ...
    async answerAsk(request) {
        let sql = `UPDATE discuss SET TeaReContent='${request.TeaReContent}' WHERE Id=${request.Id}`;
        return mysql.createConnection(DBConfig).then(conn => {
            var result = conn.query(sql);
            conn.end();
            return result;
